...
static int delegate_work_to_processes(const int *temperatures,
    ...
    while (counter < max_pid) {
        ...
        if (pid == 0) {
            max_jumps[counter_copy] =
                find_max_temperature_jump(temperatures, temperature_count,
                                          left_bound_copy, right_bound_copy);
            exit(0);
        }
    }
...
int work(const vector_t *v) {
    ...
    int *temperatures =
        mmap(NULL, v->size * sizeof(int), PROT_READ | PROT_WRITE,
             MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    ...
    int result =
        delegate_work_to_processes(temperatures, v->size, max_pid, step);
    return result;
}
